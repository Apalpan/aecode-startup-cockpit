...
@media(max-width:460px){.kstrip,.kpi-grid,.out-grid,.mm-grid,.coh-sum,.tesis-grid{grid-template-columns:1fr}}
</style>
</head>
<body>
...
add(r'''<footer><div class="foot-in">
  ...
  <div class="foot-links"><a href="#tesis">Tesis</a><a href="#hybrid">Híbrido</a><a href="#map">Mapa</a><a href="#projection">Proyección</a></div>
  <div class="foot-copy">v4 &middot; 2026 &middot; data editable · supuestos marcados · iteramos</div>
</div></footer>
''')
...
function renderAll(){
  var C=compute();
  renderVerdict(C);renderSummary(C);renderHybrid(curMode);renderModels();renderPrograms();renderFinancials();renderCohorts();renderChannels();renderKPI();renderAgenda();renderWins();
...
function initNav(){
  ...
  var ids=["summary","tesis","hybrid","map","financials","cohorts","growth","projection","health","focus"];
...
function boot(){
  ...
  stamp();renderAll();renderMap();initHybToggle();initSim();initNav();initReveal();initParticles();
  ...
  document.addEventListener("keydown",function(e){if(e.key==="Escape"){hidePop();endTour();resetMap();}});
